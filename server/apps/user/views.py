...
class send_password_reset_email(APIView):
    ...
    def post(self, request, format=None):
        ...
        reset_url = f'http://firudev.com/password/reset/confirm'
        ...
        body = f"Para cambiar tu contraseña ingresa a este link {reset_url} y luego inicia sesión."
        data = {
            'Messages': [
                {
                "From": {
                    "Email": os.environ.get('AWS_EMAIL'),
                    "Name": "Firu Dev Support"
                },
                "To": [
                    {
                    "Email": user.email,
