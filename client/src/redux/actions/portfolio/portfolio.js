...
export const get_portfolio_list_page = (p) => async dispatch =>{
    ...
    try{
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/portfolio/list?p=${p}`, config)
...
export const get_project_detail = (slug) => async dispatch => {
    ...
    try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/portfolio/project/${slug}`, config)
...
export const get_author_portfolio_list = () => async dispatch =>{
    ...
    try{
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/portfolio/author_projects`, config)
        if (res.status === 200){
            dispatch ({
...
export const get_author_portfolio_list_page = (p) => async dispatch =>{
    ...
    try{
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/portfolio/author_projects?p=${p}`, config)
